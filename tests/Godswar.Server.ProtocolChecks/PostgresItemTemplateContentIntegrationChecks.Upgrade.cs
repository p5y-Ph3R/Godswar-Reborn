        ...
        string originalRevision)
    {
        ...
        try
        {
            ...
            Check.True(
                ...
                "startup advances a v1 item pointer to a new v7 release");
            Check.Equal(
                v1Before,
                await ReadRevisionFingerprintAsync(dataSource, v1Revision),
                "v1-to-v7 upgrade never mutates the sealed v1 row or definition");
            ...
            Check.Equal(
                upgraded.Revision,
                repeated.Revision,
                "v1-to-v7 upgrade is idempotent after pointer advancement");
            Check.True(
                !repeated.Created,
                "idempotent v7 publication does not recreate content");
        }
        finally
        {
        ...
        await using var command = dataSource.CreateCommand("""
            SELECT
                ...
                NOT EXISTS (
                    ...
                    WHERE new_definition.revision = @v7Revision
                ),
                (
                    SELECT count(*)::integer
                    FROM item_template_content_definitions definition
                    WHERE definition.revision = @v7Revision
            ...
            WHERE release.revision = @v7Revision;
            """);
        command.Parameters.AddWithValue("v1Revision", v1Revision);
        command.Parameters.AddWithValue("v7Revision", v7Revision);
        command.Parameters.Add(new NpgsqlParameter(
            "skillBookIds",
            NpgsqlDbType.Array | NpgsqlDbType.Integer)
        {
