...
internal sealed partial class JsonGameStore
{
        ...
        bool requireEmptyEquipmentSlot = false)
    {
        ...
        try
        {
            ...
            if (item.IsEmpty
                || kitBagEntry == "[]"
                || !EquipmentSlots.TryGetAuthoritativeSlot(Godswar.Server.ProtocolChecks.TestItemContent.Catalog, item.Id, out var defaultEquipmentSlot))
            {
                return null;
            }

            var equipmentSlot = EquipmentSlots.ResolveSlotForItem(Godswar.Server.ProtocolChecks.TestItemContent.Catalog,
                item.Id,
            ...
            var equipEligibility = EquipmentEligibility.ValidateEquip(Godswar.Server.ProtocolChecks.TestItemContent.Content,
                character.Profession,
                character.Level,
                character.Equipment,
                item.Id,
    ...
    public async Task<KitBagItemGrantResult> AddForgingMaterialAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (!Godswar.Server.ProtocolChecks.TestItemContent.Catalog.Materials
                .TryResolveDeveloper(itemId, out var material))
        {
    ...
    public async Task<KitBagItemGrantResult> AddDeveloperMountAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (!Godswar.Server.ProtocolChecks.TestItemContent.Content
                .DeveloperMounts.TryResolveGrantable(itemId, out _))
        {
            throw new ArgumentOutOfRangeException(
                nameof(itemId),
