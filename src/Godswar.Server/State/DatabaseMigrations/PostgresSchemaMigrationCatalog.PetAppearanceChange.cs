...
{
    private static PostgresSchemaMigration CreatePetAppearanceChange() =>
        new(
            "20260812_086_pet_appearance_change",
            "Authorize durable Magic Jade pet appearance changes",
            """
            ALTER TABLE public.pet_operation_audit
                ADD CONSTRAINT ck_pet_operation_audit_operation_v6
                CHECK (
                    operation IN (
                        'owner_merge',
                        'pet_merge',
            ...
            COMMENT ON VIEW
                public.pet_content_magic_jade_appearance_groups IS
                'Versioned stock-client Magic Jade appearance mapping joined to the deputy-species Merge cap policy used by durable appearance changes.';
            """);
}
