...
{
    private const string ItemContentV6InsertGuardsSql = """
        ...
        BEGIN
            ...
            IF release.sealed_at IS NOT NULL THEN
                RAISE EXCEPTION
                    ...
                    NEW.revision;
            END IF;
            IF release.manifest_version NOT IN (2, 3, 4, 5, 6) THEN
                RAISE EXCEPTION
                    'item policy requires manifest version 2 through 6';
            END IF;
        ...
        BEGIN
            ...
            IF release.sealed_at IS NOT NULL THEN
                RAISE EXCEPTION
                    ...
                    NEW.revision;
            END IF;
            IF release.manifest_version NOT IN (3, 4, 5, 6)
               OR release.material_policy_count <= 0 THEN
                RAISE EXCEPTION
                    'item material policy requires manifest version 3 through 6';
            END IF;
            IF release.manifest_version = 3
               AND NEW.recipe_kind IS NOT NULL THEN
                RAISE EXCEPTION
                    'item material recipes require manifest version 4 through 6';
            END IF;
        ...
        BEGIN
            ...
            IF release.sealed_at IS NOT NULL THEN
                RAISE EXCEPTION
                    ...
                    NEW.revision;
            END IF;
            IF release.manifest_version NOT IN (5, 6) THEN
                RAISE EXCEPTION
                    'Holy Suit policy requires item manifest version 5 or 6';
            END IF;
