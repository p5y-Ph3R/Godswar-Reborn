...
internal static class GearEnhancerProtocol
{
    ...
    {
        // The stock Gear Mentor owns all nine NpcFunBreak menu positions.
        // Operations which are not implemented are still advertised so the
        // original dialog has no gaps; the handler answers them with native
        // result 999 ("Temporarily Disabled").
        return PacketBuilder.NpcFunctionActionResponse(
            npcId,
            DialogIndex,
            1,
            EnhanceAttributeSubId,
            AddAttributeSubId,
            4,
            5,
            DeleteAttributesSubId,
    ...
    private static byte[] BuildOriginInitialMenuResponse(uint npcId)
    {
        return PacketBuilder.NpcFunctionActionResponse(
            npcId,
            OriginDialogIndex,
    ...
    public static bool TryBuildOriginInitialMenuResponse(
        string npcKey,
        uint npcId,
        int dialogIndex,
        int subId,
    ...
    {
        response = [];
        if (!IsOriginEnhancerEndpoint(npcKey, npcId) ||
            dialogIndex != OriginDialogIndex ||
            subId != InitialMenuRequestSubId)
        {
            return false;
        }

        response = BuildOriginInitialMenuResponse(npcId);
        return true;
    }
