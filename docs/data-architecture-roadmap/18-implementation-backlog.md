...
| B20 - Legacy retirement **(B20A-G complete locally; B20H live window pending)** | Finish migration | broad store, projections/content, zero-use gate | Approved seven-day all-replica zero-use record | One production authority | install, parity, validator negatives | legacy calls zero | Compatibility release/archive | Large | Medium |
