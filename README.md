...
- `tools/PatchClientAvatarPreviewGuard.ps1` repairs the stale LOGIN avatar-initialization flag and guards both known native selection-avatar builders. The installed client changes exactly 169 bytes, has SHA-256 `1BBD41D4E148E040B363D2A83D36CD326A2C2CFE1EA44E08DA6B2680CA1BB329`, and can be restored from `C:\Reborn\backups\origin-avatar-preview-guard-Apply-20260722-135817166\Origin.exe`. The companion `Net.dll` gate now holds the exact one-character preview message while those resources load and delivers it automatically when ready. Root-cause addresses and rollback details are in [`docs/client-avatar-preview-crash-fix.md`](docs/client-avatar-preview-crash-fix.md); the loading/ownership contract is in [`docs/client-avatar-preview-loading-gate.md`](docs/client-avatar-preview-loading-gate.md).
