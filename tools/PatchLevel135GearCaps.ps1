...
$targetTypes = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
@(
    'head',
    'amulet',
    'glove',
    ...
    'ring',
    'shield'
) | ForEach-Object { [void]$targetTypes.Add($_) }
...
function Test-TargetGear([string]$line) {
    $type = Get-AttributeValue $line 'Type'
    if ([string]::IsNullOrWhiteSpace($type) -or -not $targetTypes.Contains($type)) {
        return $false
    }
...
$backupRoot = Join-Path (Resolve-Path '.').Path "backups\level135plus-gear-caps-$timestamp"
