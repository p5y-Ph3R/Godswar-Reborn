...
function Test-TargetLeggings([System.Xml.XmlElement]$node) {
    if ($node.GetAttribute('Type') -ne 'leggins') {
        return $false
    }
...
function Patch-ItemBaseAttribute([string]$path) {
    ...
    foreach ($node in $document.SelectNodes('/ItemBaseAttribute//*[@ID]')) {
        if (-not (Test-TargetLeggings $node)) {
            continue
        }
...
$backupRoot = Join-Path (Resolve-Path '.').Path "backups\level135plus-leggings-caps-$timestamp"
