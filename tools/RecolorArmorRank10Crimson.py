...
def recolor_pixel(pixel: bytes) -> bytes:
    ...
    # Preserve the near-black dominance, but move the hue into crimson.
    new_red = round(150.0 * glow)
    new_green = round(3.0 * glow)
    new_blue = round(24.0 * glow)
...
def main() -> None:
    backup_dir = BACKUP_ROOT / f"armor-rank10-near-black-crimson-{datetime.now():%Y%m%d-%H%M%S}"
