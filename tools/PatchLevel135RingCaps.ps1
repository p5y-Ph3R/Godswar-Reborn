...
function Test-TargetRing([System.Xml.XmlElement]$node) {
    if ($node.GetAttribute('Type') -ne 'ring') {
        return $false
    }
...
$backupRoot = Join-Path (Resolve-Path '.').Path "backups\level135plus-ring-caps-$timestamp"
