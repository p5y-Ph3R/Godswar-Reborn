...
inline void BuildMovePacket(
    ...
    Write16(
        ...
        static_cast<std::uint16_t>(
            sourceBagSlot / LegacyKitBagSlotsPerPage));
    Write16(
        packet + 10,
        static_cast<std::uint16_t>(
            sourceBagSlot % LegacyKitBagSlotsPerPage));
...
inline bool DescribeMove(
    ...
    LegacyPacketDescriptor* descriptor) {
    std::uint8_t
        packet[LegacyKitBagItemMoveDetailedPacketBytes]{};
    BuildMovePacket(
        packet,
        packetBytes,
        sourceBagSlot,
